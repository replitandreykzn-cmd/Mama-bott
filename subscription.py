...
async def buy_premium(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    keyboard = [[InlineKeyboardButton("◀️ Назад", callback_data="subscription")]]
    await query.edit_message_text(
        f"💳 *Покупка Premium*\n\n"
        f"Стоимость: *{PREMIUM_PRICE}*\n"
        f"Оплата: карта РФ, СБП, Сбер\n\n"
        f"Напишите мне в Telegram, я помогу оформить подписку:\n"
        f"👉 [Написать](tg://user?id={OWNER_TG_ID})\n\n"
